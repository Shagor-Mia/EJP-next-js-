...
import { createUser, findUserByEmail, createEvent, deleteEvent as dbDeleteEvent } from './data';
...
export async function signInWithCredentials(prevState: AuthState | undefined, formData: FormData) {
  try {
...
export async function registerUser(prevState: AuthState | undefined, formData: FormData) {
  ...
  try {
    await signIn('credentials', { email, password, redirectTo: '/manage-events' });
